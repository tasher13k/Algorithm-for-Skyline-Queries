...
namespace calculation {
    bool boundedNNsearch(point_type &nn, rtree_type &tree, point_type region_border_point) {
        ...
        tree.query(bgi::nearest(point_type{0, 0}, 1) &&
                   bgi::covered_by(bg::model::box({0, 0}, region_border_point)), std::back_inserter(result_nn));
        if (result_nn.empty()) {
            return false;
        }
        nn = result_nn.front();
    ...
    vector_type get_skyline(const vector_type &all_points) {
        vector_type result_nn;
        queue to_do_list;
        to_do_list.emplace(constants::max_value_of_coordinate,
                           constants::max_value_of_coordinate); // only one element - all space
